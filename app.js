(() => {
  ...
  const liveHours = clockIn => Math.max(0, (Date.now() - new Date(clockIn).getTime()) / 3600000);
  ...
  function showStaleNotice(n){
    if(!n)return;
    const ci=new Date(n.clockIn),co=new Date(n.clockOut);
    $('staleNoticeText').textContent=`You clocked in on ${ci.toLocaleDateString()} at ${ci.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})} but the system never saw a Clock Out, so it closed that shift automatically after 12 hours (at ${co.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}). It's been paid a standard day provisionally, and your manager can review or adjust it.`;
    $('staleNotice').classList.remove('hide');
  }
  async function loadEmployee(){
    ...
    try{empCache=await api(`/api/employee?start=${encodeURIComponent(start.toISOString())}&end=${encodeURIComponent(end.toISOString())}`);employeeIdentity={id:empCache.employee.id,name:empCache.employee.name};renderEmployee();}
  ...
  function renderEmployee(){
    ...
    $('shift').classList.toggle('hide',!open);$('earlyNoteWrap').classList.toggle('hide',!open);$('cancelClockInBtn').classList.toggle('hide',!open);$('in').disabled=!!open;$('out').disabled=!open;
    ...
    if(empCache.staleNotice)showStaleNotice(empCache.staleNotice);
    ...
    const a=empCache.week.days.slice(),rate=Number(p.hourly_wage||0),names=['MON','TUE','WED','THU','FRI','SAT','SUN'];
    const now=new Date(),curStart=ws(now),todayIdx=Math.floor((ds(now)-ds(curStart))/86400000);
    if(open&&start.getTime()===curStart.getTime()&&todayIdx>=0&&todayIdx<7)a[todayIdx]+=liveHours(open.clock_in);
    let total=0;
  ...
  async function initEmployee(){
    ...
    $('dismissStaleBtn').onclick=()=>$('staleNotice').classList.add('hide');
    ...
    $('out').onclick=async()=>{try{stat('Checking GPS and location limit…');const c=await geo();const j=await api('/api/clock-out',{method:'POST',body:JSON.stringify({lat:c.latitude,lng:c.longitude,note:$('earlyNote').value,message:$('earlyMessage').value})});$('earlyMessage').value='';const extra=j.pendingApproval?' Pending manager review.':'';stat(`Clock Out accepted. Distance from Clock In: ${Number(j.distance).toFixed(2)} miles.${extra}`,'ok');await loadEmployee();}catch(e){if(e.status===401)showEmployeeLogin();else stat(e.message,'bad');}};
    $('cancelClockInBtn').onclick=async()=>{
      if(!confirm('Cancel this Clock In? This shift will not be paid, and you can clock in again once at the right location.'))return;
      try{const j=await api('/api/cancel-clock-in',{method:'POST'});stat(j.message,'ok');await loadEmployee();}catch(e){if(e.status===401)showEmployeeLogin();else stat(e.message,'bad');}
    };
  ...
  function renderPeople(){
    ...
    $('approvedList').innerHTML=managerPeople.map(p=>`<div class="pill"><b>${esc(p.name)}</b><span>· $${Number(p.hourly_wage).toFixed(2)}/hr · ${p.email?esc(p.email)+' · ':''}${p.has_pin?'PIN set':'PIN needed'}${p.has_open_shift?' · <b class="onClock">on the clock</b>':''}</span><button data-pin="${p.id}">${p.has_pin?'Reset PIN':'Set PIN'}</button>${p.has_open_shift?`<button class="clockout" data-clockout="${p.id}">Clock Out</button>`:''}<button class="remove" data-remove="${p.id}">Remove</button></div>`).join('')||'<span class="small">No active employees yet.</span>';
    ...
    document.querySelectorAll('[data-clockout]').forEach(b=>b.onclick=async()=>{
      if(!confirm('Clock this employee out now? This closes their active shift.'))return;
      try{await api('/api/manager-clock-out',{method:'POST',body:JSON.stringify({employeeId:Number(b.dataset.clockout)})});$('employeeMsg').textContent='Employee clocked out.';await loadPeople();await mgrRender();}catch(e){$('employeeMsg').textContent=e.message;}
    });
  ...
  async function loadSettings(){
    try{
      ...
      $('radiusSetting').value=s.clock_out_radius_miles??3;$('projectMinSetting').value=s.project_completed_min_paid_hours??8;
      $('thresholdSetting').value=s.full_day_round_threshold_hours??7.75;$('maxEmployeesSetting').value=s.max_active_employees??100;
      $('siteLabel').value=s.site_label||'';$('siteLat').value=s.site_lat||'';$('siteLng').value=s.site_lng||'';
    }catch(e){$('settingsMsg').textContent=e.message;}
  }
  function renderExceptions(ex){
    const parts=[];
    if(ex.radiusRejections?.length){
      parts.push(`<div class="exGroup"><h3>Out-of-radius attempts</h3>`+ex.radiusRejections.map(r=>`<div class="exRow bad"><b>${esc(r.name)}</b><span>${new Date(r.created_at).toLocaleString()} · ${Number(r.distance_miles).toFixed(2)} mi away — not clocked out</span></div>`).join('')+`</div>`);
    }
    if(ex.overtime?.length){
      parts.push(`<div class="exGroup"><h3>Overtime</h3>`+ex.overtime.map(r=>`<div class="exRow"><b>${esc(r.name)}</b><span>${new Date(r.date).toLocaleDateString()} · worked ${r.actualHours.toFixed(2)}h, ${r.overBy.toFixed(2)}h over the standard day</span></div>`).join('')+`</div>`);
    }
    if(ex.earlyPersonal?.length){
      parts.push(`<div class="exGroup"><h3>Early / personal departures</h3>`+ex.earlyPersonal.map(r=>`<div class="exRow"><b>${esc(r.name)}</b><span>${new Date(r.date).toLocaleDateString()} · ${esc(r.reason)} · paid ${r.paidHours.toFixed(2)}h${r.message?' · “'+esc(r.message)+'”':''}</span></div>`).join('')+`</div>`);
    }
    if(ex.pendingApproval?.length){
      parts.push(`<div class="exGroup"><h3>Pending your approval</h3>`+ex.pendingApproval.map(r=>`<div class="exRow pending"><b>${esc(r.name)}</b><span>${new Date(r.date).toLocaleDateString()} · ${esc(r.reason)} · worked ${r.actualHours.toFixed(2)}h, provisionally paid ${r.provisionalPaidHours.toFixed(2)}h${r.message?' · “'+esc(r.message)+'”':''}</span><div class="exActions"><button type="button" class="secondary" data-approve="${r.shiftId}">Approve ${r.provisionalPaidHours.toFixed(2)}h</button><input type="number" class="adjustHours" min="0" max="24" step="0.25" placeholder="hours" data-shift-adjust="${r.shiftId}"><button type="button" class="secondary" data-adjust="${r.shiftId}">Adjust</button></div></div>`).join('')+`</div>`);
    }
    $('exceptionsList').innerHTML=parts.join('')||'<p class="muted small">Nothing needs attention for this period.</p>';
    document.querySelectorAll('[data-approve]').forEach(b=>b.onclick=async()=>{try{await api('/api/shift-approve',{method:'PATCH',body:JSON.stringify({shiftId:Number(b.dataset.approve),action:'approve'})});await mgrRender();}catch(e){alert(e.message);}});
    document.querySelectorAll('[data-adjust]').forEach(b=>b.onclick=async()=>{
      const shiftId=Number(b.dataset.adjust),input=document.querySelector(`[data-shift-adjust="${shiftId}"]`),hours=Number(input?.value);
      if(!Number.isFinite(hours)||hours<0||hours>24)return alert('Enter a valid number of hours (0–24).');
      try{await api('/api/shift-approve',{method:'PATCH',body:JSON.stringify({shiftId,action:'adjust',paidHours:hours})});await mgrRender();}catch(e){alert(e.message);}
    });
  }
  async function mgrRender(){
    ...
    try{
      ...
      const badge=x=>{const t=[];if(x.cancelled)t.push('<span class="tag bad">cancelled</span>');if(x.auto_clocked_out)t.push('<span class="tag warn">auto 12h</span>');if(x.closed_by_manager)t.push('<span class="tag">by manager</span>');if(x.approval_status==='pending')t.push('<span class="tag warn">pending</span>');if(x.approval_status==='adjusted')t.push('<span class="tag">adjusted</span>');return t.join(' ');};
      $('rows').innerHTML=rows.map(x=>{const actual=Math.max(0,(new Date(x.clock_out||Date.now())-new Date(x.clock_in))/3600000),w=Number(x.hourly_wage||0),ci=new Date(x.clock_in),co=x.clock_out?new Date(x.clock_out):null;const paid=Number(x.paid_hours||0);const paidDisplay=x.clock_out?paid.toFixed(2):'—';const outLoc=x.closed_by_manager?'Manager closed — no employee GPS':loc(x.clock_out_lat,x.clock_out_lng);return `<tr><td><b>${esc(x.name)}</b></td><td>${ci.toLocaleDateString()}</td><td>${ci.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</td><td>${co?co.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}):'Open'}</td><td>${actual.toFixed(2)}</td><td>${paidDisplay}</td><td>${loc(x.clock_in_lat,x.clock_in_lng)}</td><td>${outLoc}</td><td>${esc(x.clock_out_note||'—')}${x.clock_out_message?`<div class="small muted">"${esc(x.clock_out_message)}"</div>`:''} ${badge(x)}</td><td>${Number(x.rejected_count||0)}</td></tr>`;}).join('')||'<tr><td colspan="10">No records.</td></tr>';
  ...
  async function startManagerApp(){
    ...
    $('saveSettings').onclick=async()=>{
      try{
        const j=await api('/api/settings',{method:'PATCH',body:JSON.stringify({
          clock_out_radius_miles:Number($('radiusSetting').value),
          project_completed_min_paid_hours:Number($('projectMinSetting').value),
          full_day_round_threshold_hours:Number($('thresholdSetting').value),
          max_active_employees:Number($('maxEmployeesSetting').value),
          site_lat:$('siteLat').value,site_lng:$('siteLng').value,site_label:$('siteLabel').value
        })});
        const s=j.settings||{};$('radiusSetting').value=s.clock_out_radius_miles;$('projectMinSetting').value=s.project_completed_min_paid_hours;$('thresholdSetting').value=s.full_day_round_threshold_hours;$('maxEmployeesSetting').value=s.max_active_employees;$('siteLabel').value=s.site_label||'';$('siteLat').value=s.site_lat||'';$('siteLng').value=s.site_lng||'';
        $('settingsMsg').textContent='Settings saved.';
      }catch(e){$('settingsMsg').textContent=e.message;}
    };
    $('useMyLocationBtn').onclick=async()=>{
      try{$('settingsMsg').textContent='Getting your current location…';const c=await geo();$('siteLat').value=c.latitude.toFixed(6);$('siteLng').value=c.longitude.toFixed(6);$('settingsMsg').textContent='Location filled in below — click Save Settings to apply it.';}
      catch(e){$('settingsMsg').textContent=e.message;}
    };
  ...
  async function initManager(){
    ...
    $('forgotManagerBtn').onclick=()=>{$('managerRecovery').classList.toggle('hide');$('managerRecoveryMsg').textContent='Your request to reset the password has been initiated. A secure link will be sent to the admin email on file.';};
