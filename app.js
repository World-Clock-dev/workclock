(() => {
  ...
  async function mgrRender(){
    ...
    try{
      ...
      $('rows').innerHTML=rows.map(x=>{const actual=Math.max(0,(new Date(x.clock_out||Date.now())-new Date(x.clock_in))/3600000),w=Number(x.hourly_wage||0),ci=new Date(x.clock_in),co=x.clock_out?new Date(x.clock_out):null;return `<tr><td><b>${esc(x.name)}</b></td><td>${ci.toLocaleDateString()}</td><td>${ci.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</td><td>${co?co.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}):'Open'}</td><td>${actual.toFixed(2)}</td><td>${loc(x.clock_in_lat,x.clock_in_lng)}</td><td>${loc(x.clock_out_lat,x.clock_out_lng)}</td><td>${esc(x.clock_out_note||'—')}${x.clock_out_message?`<div class="small muted">"${esc(x.clock_out_message)}"</div>`:''} ${badge(x)}</td><td>${Number(x.rejected_count||0)}</td></tr>`;}).join('')||'<tr><td colspan="9">No records.</td></tr>';
