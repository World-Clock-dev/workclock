(() => {
  ...
  const paidMinutes = x => Number.isFinite(Number(x?.paid_minutes)) ? Number(x.paid_minutes) : actualMinutes(x || {});
  ...
  const safeDays = v => Array.isArray(v) ? v : Array(7).fill(0);
  ...
  function renderEmployee(){
    ...
    for(const x of Array.isArray(empCache.shifts)?empCache.shifts:[]){ const i=Math.floor((ds(new Date(x.clock_in))-ds(start))/86400000); if(i>=0&&i<7) mins[i]+=paidMinutes(x); }
    let total=0;
    $('days').innerHTML=names.map((name,i)=>{const dt=new Date(start);dt.setDate(dt.getDate()+i);const dayMins=Number(mins[i]||0);total+=dayMins;return `<div class="day"><div><div class="dn">${name}</div><div class="dd">${dt.toLocaleDateString([],{month:'numeric',day:'numeric'})}</div></div><div><div class="dh">${(dayMins/60).toFixed(2)}h</div><div class="dm">${money(dayMins,rate)}</div><div class="dayEarnedLabel">earned</div></div></div>`;}).join('');
    $('weekTotal').textContent=(total/60).toFixed(2); $('weekEarned').textContent=money(total,rate);
    const now=new Date(),curStart=ws(now);
    if(start.getTime()===curStart.getTime()){const idx=Math.floor((ds(now)-ds(curStart))/86400000),today=Number(mins[idx]||0);$('hoursToday').textContent=(today/60).toFixed(2);$('earnedToday').textContent=`${money(today,rate)} earned today`;$('hoursWeek').textContent=(total/60).toFixed(2);$('earnedWeek').textContent=`${money(total,rate)} earned this week`;} else {$('hoursToday').textContent='0.00';$('earnedToday').textContent='$0.00 earned today';$('hoursWeek').textContent=(total/60).toFixed(2);$('earnedWeek').textContent=`${money(total,rate)} earned this week`;}
  }
  ...
  async function mgrRender(){
    ...
    try{
      ...
      $('rows').innerHTML=rows.map(x=>{const am=actualMinutes(x),pm=paidMinutes(x),w=Number(x.hourly_wage||0),ci=new Date(x.clock_in),co=x.clock_out?new Date(x.clock_out):null;let action='—';if(!co)action=`<button class="tableBtn" data-manager-out="${x.id}">Clock Out</button>`;if(x.payment_status==='pending_approval')action+=` <button class="tableBtn approve" data-approve="${x.id}">Approve 8h</button>`;return `<tr><td><b>${esc(x.name)}</b></td><td>${esc(x.job_title||'Employee')}</td><td>${ci.toLocaleDateString([],{weekday:'long'})}</td><td>${ci.toLocaleDateString()}</td><td>${ci.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</td><td>${co?co.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}):'Open'}</td><td>${fmtMins(am)}</td><td>${fmtMins(pm)}</td><td>${esc(statusLabel(x.payment_status))}</td><td>${money(pm,w)}</td><td>${loc(x.clock_in_lat,x.clock_in_lng)}</td><td>${loc(x.clock_out_lat,x.clock_out_lng)}</td><td>${esc(x.clock_out_note||'—')}</td><td>${esc(x.clock_out_message||'—')}</td><td>${esc(x.clock_out_source||'—')}</td><td>${action}</td></tr>`;}).join('')||'<tr><td colspan="16">No shift records for this period.</td></tr>';
      document.querySelectorAll('[data-manager-out]').forEach(b=>b.onclick=async()=>{const reason=prompt('Manager Clock Out reason:','Manager clock-out');if(reason===null)return;const message=prompt('Optional message:','')||'';try{b.disabled=true;await managerAction(b.dataset.managerOut,'clock_out',reason,message);await mgrRender();}catch(e){alert(e.message);b.disabled=false;}});
      document.querySelectorAll('[data-approve]').forEach(b=>b.onclick=async()=>{if(!confirm('Approve this shift for the full 8-hour payment?'))return;try{b.disabled=true;await managerAction(b.dataset.approve,'approve_full_shift','Manager approved full shift');await mgrRender();}catch(e){alert(e.message);b.disabled=false;}});
      ...
      const weekly=Array.isArray(j.weekly)?j.weekly:[];
      if(mode==='week'){
