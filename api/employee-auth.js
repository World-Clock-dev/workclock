...
export default async function handler(req,res){
  try{
    ...
    if(req.method==='POST'){
      ...
      const rows=await sql`SELECT id,name,title,pin_hash,failed_pin_attempts,pin_locked_until FROM employees WHERE normalized_name=${name} AND active=true LIMIT 1`;
      ...
      return json(res,200,{ok:true,employee:{id:e.id,name:e.name,title:e.title}});
    }
