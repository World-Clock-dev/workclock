...
function validWage(value) { const n = Number(value); return Number.isFinite(n) && n >= 0 && n <= 100000 ? n : null; }
function validId(value) { const n = Number(value); return Number.isInteger(n) && n > 0 ? n : null; }
function validTitle(value) {
  if (value === undefined || value === null || String(value).trim() === '') return null;
  const t = String(value).trim().replace(/\s+/g, ' ');
  return t.length <= 80 ? t : undefined;
}
function validPin(value) { const pin = String(value || '').trim(); return /^\d{4}$/.test(pin) ? pin : null; }
...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'POST') {
      const b=req.body||{}, name=String(b.name||'').trim().replace(/\s+/g,' '), wage=validWage(b.wage??0), pin=validPin(b.pin), email=validEmail(b.email), title=validTitle(b.title);
      if (email===undefined) return json(res,400,{error:'Enter a valid employee email or leave it blank.'});
      if (title===undefined) return json(res,400,{error:'Employee title must be 80 characters or fewer.'});
      ...
      if (!pin) return json(res,400,{error:'Enter a 4-digit employee PIN.'});
    ...
    if (req.method === 'PATCH') {
      ...
      const wage=validWage(b.wage); if(wage===null)return json(res,400,{error:'Invalid wage.'});
